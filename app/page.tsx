...
import { calculateVotingPower, calculateFullVoteValue, calculateMaxVoteValue, getEffectiveHivePower, parseReputation, calculateResourceCredits, toNumber } from '@/lib/votemath';
...
export default function Home() {
  const [accountName, setAccountName] = useState('mantecurated');
  const [inputValue, setInputValue] = useState('mantecurated');
  ...
  const fetchAccountData = async (username: string) => {
    ...
    try {
      ...
      console.log('Account HP:', hivePower.toFixed(2));
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (inputValue.trim()) {
      setAccountName(inputValue.trim());
    }
  };
  ...
  return (
    <main className="min-h-screen p-6 md:p-10">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="mb-10">
          <h1 className="text-5xl md:text-6xl font-bold mb-3" style={{ fontFamily: 'Syne, sans-serif' }}>
            <span className="glow-red">ManteCurated</span>
            <span className="text-[var(--text-secondary)] ml-2">LIVE</span>
          </h1>
          <p className="text-[var(--text-muted)] text-sm uppercase tracking-widest">Hive Blockchain Curation Terminal</p>
          <div className="w-20 h-1 bg-gradient-to-r from-[var(--hive-red)] to-transparent mt-4"></div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="mb-10">
          <div className="flex gap-3 max-w-2xl">
            ...
            <button
              type="submit"
              disabled={loading}
              className="px-8 py-4 bg-[var(--hive-red)] hover:bg-red-600 disabled:bg-[var(--border-primary)] disabled:cursor-not-allowed rounded-xl font-bold transition-all relative overflow-hidden group"
            >
        ...
        {loading && (
          <div className="flex items-center justify-center py-20">
            <div className="text-center">
              <div className="inline-block w-16 h-16 border-4 border-[var(--border-primary)] border-t-[var(--hive-red)] rounded-full animate-spin mb-4"></div>
