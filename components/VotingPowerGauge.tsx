      ...
      <div className="flex items-center justify-center mb-8">
        <div className="relative w-72 h-72">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
            <div className="text-6xl font-bold glow-red data-value" style={{ fontFamily: 'Syne, sans-serif' }}>
              {votingPower.toFixed(1)}
            </div>
            <div className="text-xl font-semibold text-[var(--hive-red)] -mt-1">%</div>
            <div className="text-xs uppercase tracking-wider text-[var(--text-muted)] mt-2">Voting Power</div>
          </div>

          {/* Decorative ring */}
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
            <div className="w-[190px] h-[190px] rounded-full border border-[var(--border-primary)] opacity-30"></div>
          </div>
        </div>
      </div>

      {/* Vote Value */}
      <div className="bg-[var(--bg-secondary)] rounded-lg p-5 border border-[var(--border-primary)] hover:border-[var(--hive-red)] transition-colors">
        <div className="text-xs uppercase tracking-wider text-[var(--text-muted)] mb-2">Full Weight Vote Value</div>
        <div className="text-3xl font-bold text-[var(--hive-red)] data-value mb-1">
          ${maxVoteValue.toFixed(3)}
        </div>
